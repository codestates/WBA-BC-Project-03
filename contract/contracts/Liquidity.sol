...
constructor (address _token) ERC20("lpToken","LP"){
  ...
  function addLiquidity(uint256 _maxToken) public payable{
    ...
    if (totalLiquidity > 0){
      ...
      // 가지고 있던 이더리움의 개수 만큼 토큰을 발행한다.(Uniswap v1에서 단순하게 하기 위해 공급된 이더리움 개수와 맞춰서 LP토큰을 발행 함)
  ...
  // _minToken : 슬리피지가 포함된 값, 최소 사용자에게 줘야하는 토큰, 사용자가 입력한 슬리피지 값 / 7.5랑 msg.sender
  ...
  function swapTokenToCoin(uint256 _tokenAmount, uint256 _minCoin) public payable{
    ...
    // 이더를 보낸다. 함수를 호출한 사용자에게 CPMM 슬리피지가 적용된 양 만큼
...
// _minTokenBought : 사용자가 받아야할 최소 토큰
// _minTokenBought : 사용자가 받아야할 최소 코인
